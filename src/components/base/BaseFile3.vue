<template>
  ...
  async function rotate(deg) {
    ...
    function drawRotated(degrees) {
      ...
      const img = new Image()
      img.src = canvas.toDataURL('image/jpeg') || images.value[currentIndex.value].url
    ...
    // let image = modalImg.value
    // image.style.transform = `rotate(${(this.deg = (this.deg || 0) + deg)}deg)`
